...
export default function RedeemForm() {
  ...
  const isValidAmount = useMemo(() => {
    ...
    try {
      ...
      return amt > 0n && amt <= BigInt(balance as bigint);
    } catch {
      return false;
    }
  ...
  const handleMax = () => {
    if (balance !== undefined && balance !== null && decimals !== undefined) {
      setAmount(formatUnits(BigInt(balance as bigint), Number(decimals)));
    }
  };
      ...
      <div className="bg-gray-800 rounded-lg p-6 space-y-4">
        <div className="flex justify-between items-center">
          ...
          <div className="text-sm text-gray-400">
            Balance: {balance !== undefined && balance !== null && decimals !== undefined ? formatUnits(BigInt(balance as bigint), Number(decimals)) : '0'} {symbol || 'plUSD'}
          </div>
        </div>
